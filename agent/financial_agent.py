...
def _total_debt(selected_facts: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    if explicit:
        f = explicit[0]
        return {
            "metric": "total_debt", "status": "reported", "answer": f["value"],
    ...
    for fact in components:
        key = (
            str(fact.get("label", "")).lower(), str(fact.get("section_context", "")).lower(),
            fact.get("value"), fact.get("page"), fact.get("period"),
        )
        if key not in seen:
            seen.add(key)
            unique.append(fact)
    ...
    if commercial and len(term) >= 2:
        ...
        return {
            ...
            "source": {"items": chosen}, "confidence": "high",
        }
    return None
