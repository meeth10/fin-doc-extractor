...
def _total_debt(selected_facts: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    explicit = [
        f for f in facts
        if "total debt" in str(f.get("label", "")).lower()
    ]
    if explicit:
        f = explicit[0]
        return {
    ...
    components = [
        f for f in facts
        if any(
            token in str(f.get("label", "")).lower()
            or token in str(f.get("section_context", "")).lower()
    ...
    for fact in components:
        key = (
            str(fact.get("label", "")).lower(),
            str(fact.get("section_context", "")).lower(),
            fact.get("value"),
            fact.get("page"),
            fact.get("period"),
        )
    ...
    commercial = [
        f for f in unique
        if "commercial paper" in str(f.get("label", "")).lower()
    ]
    term = [
        f for f in unique
    ...
    if commercial and len(term) >= 2:
        # For identical labels, section context preserves the current vs
        # non-current distinction. Sum exactly one of each required component.
...
def answer_question(question: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Factual questions do not pay for a reasoning-model call. The deterministic
    # answer plus source provenance is authoritative; Qwen is reserved for
    # interpretation-heavy questions.
