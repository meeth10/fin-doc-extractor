      ...
      <div class="ask-shell">
        <div class="ask-hero">
          <div>
            ...
            <p>Ask about the loaded document. The resolver selects evidence first; Ollama explains only what the evidence supports.</p>
          </div>
          <div class="ollama-pill">Ollama · qwen3:14b</div>
        </div>
        <div class="ask-box">
        ...
        <div class="ask-foot">Answers are evidence-bound. A value may be reported, derived, ambiguous, or unavailable. Derived values must expose their calculation inputs.</div>
      </div>`;
  ...
  function renderAnswer(a) {
    ...
    result.innerHTML = `<article class="answer-card">
      <div class="answer-top">
        ...
        <div class="answer-meta"><span class="answer-badge ${statusClass}">${esc(String(a.status || '').toUpperCase())}</span><span class="answer-badge">${esc(String(a.confidence || '').toUpperCase())} CONFIDENCE</span>${a.period ? `<span class="answer-badge">${esc(a.period)}</span>` : ''}</div>
      </div>
